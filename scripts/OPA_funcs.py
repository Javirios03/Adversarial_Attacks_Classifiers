...
class OnePixelAttack:
    ...
    def _initialize_population(self) -> None:
        """
        Initializes the population of perturbations according to the guidelines of the One Pixel Attack paper."""
        # Uniform for positions [0.0, 1.0] 
        positions = torch.rand((self.n, 2), device=self.device)

        # RGB samples from N(128, 127), then clipped to [0, 255] and then normalized to [0, 1]
        rgb = torch.normal(
            mean=128.0, std=127.0, size=(self.n, 3), device=self.device
        )
        rgb = torch.clamp(rgb, 0.0, 255.0) / 255.0

        self.population = torch.cat((positions, rgb), dim=1)
    ...
    def _generate_new_population(self) -> None:
        ...
        # Obtain the random indices for DE
        r1 = torch.randint(0, self.n, (self.n,), device=self.device)
        r2 = torch.randint(0, self.n, (self.n,), device=self.device)
        ...
        # Assess whether children are better than parents
        better_mask = new_fitnesses > self.fitness if is_targeted else new_fitnesses < self.fitness
        # Update population and fitness values
        self.population = torch.where(
            better_mask, candidates, self.population
        )
        self.fitness = torch.where(
            better_mask, new_fitnesses, self.fitness
        )

        ## Old Code - Not optimal for Batch Processing
        # new_population = []
        # new_fitness = []
        ...
        for j in range(d):
            ...
            for i in range(epochs):
                ...
                if print_every and i % print_every == 0:
                    print(f"Pixel nº{j+1} Epoch {i}: Perturbations = {self.population}")
            ...
            # Update the perturbed image with the best perturbation found
            self.perturbed_img = self._add_perturbation(
                [best_p], self.perturbed_img
            )
